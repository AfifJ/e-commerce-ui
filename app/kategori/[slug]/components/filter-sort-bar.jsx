  ...
  return (
    <div className="bg-white border-b sticky top-0 z-40">
      <div className="container mx-auto px-4 py-4">
        <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-center flex-1 w-full lg:w-auto">
            ...
            >
              ...
              Filter
              {(priceRange.min || priceRange.max || selectedRating !== 'all') && (
                <Badge variant="secondary" className="ml-1 h-5 w-5 p-0 flex items-center justify-center text-xs">
                  {[priceRange.min || priceRange.max, selectedRating !== 'all'].filter(Boolean).length}
                </Badge>
              )}
            </Button>
          </div>
        ...
        {/* Active Filters */}
        {(searchTerm || priceRange.min || priceRange.max || selectedRating !== 'all') && (
          <div className="flex flex-wrap gap-2 mt-3">
            ...
            <Button variant="ghost" size="sm" onClick={() => handleFilterChange('clearAll', '')}>
              Hapus Semua
            </Button>
          </div>
        )}
