...
const getCurrentUserId = () => {
  ...
  try {
    ...
    // If user is authenticated and has ID, use it
    if (state.user?.id && state.isAuthenticated) {
      return state.user.id;
    }

    // Fallback ke user pertama untuk development
...
export async function getCurrentUser() {
  try {
    ...
    // Get user data from auth store directly
    try {
      const { useAuthStore } = require("@/stores/auth-store");
      const state = useAuthStore.getState();

      if (state.isAuthenticated && state.user) {
        // Use the authenticated user from auth store
        const userData = {
          id: state.user.id,
          name: state.user.name || state.user.username,
          email: state.user.email,
          phone: state.user.phone || '+62 812-3456-7890', // Fallback phone
          address: 'Jl. Default Address No. 123', // Default address
          isVerified: state.user.emailVerified || false,
          nama: state.user.name || state.user.username, // For compatibility
          whatsapp: state.user.phone || '+62 812-3456-7890', // For compatibility
          alamat: 'Jl. Default Address No. 123' // For compatibility
        }
      ...
      console.log('Auth store not available, using fallback');
    }

    // Fallback to mock user if auth store is not available or user is not authenticated
    ...
    const userData = {
      ...
      nama: user.nama, // For compatibility
      whatsapp: user.whatsapp, // For compatibility
      alamat: user.alamat // For compatibility
    }
